// Test script for fair allocation algorithm V2
// Use soft fairness constraint with high weight
...
function runFairAllocationV2() {
  console.log('=== RUGBY PLANNER - FAIR ALLOCATION V2 ===\n');
  ...
  for (let halfIdx = 0; halfIdx < halves; halfIdx++) {
    ...
    console.log('Fairness scores (higher = needs more field time):');
    const sortedByFairness = [...players].sort((a, b) => fairnessScores[b] - fairnessScores[a]);
    sortedByFairness.slice(0, 5).forEach(p => {
      console.log(`  Player ${p}: ${fairnessScores[p].toFixed(3)} (current: ${fieldCounts[p]}/${halfIdx})`);
    });
    ...
    console.log(`\nAssignments:`);
    positions.forEach(pos => {
      if (halfAssignments[pos]) {
        console.log(`  Position ${pos} -> Player ${halfAssignments[pos]}`);
      } else {
        console.log(`  Position ${pos} -> UNFILLED`);
      }
    });
