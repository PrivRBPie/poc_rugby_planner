...
create policy rugby_member_select on rugby_data
  for select to authenticated
  using (exists (select 1 from team_members tm where tm.team_id = rugby_data.team_id and tm.user_id = auth.uid()));

create policy rugby_member_insert on rugby_data
  for insert to authenticated
  with check (exists (select 1 from team_members tm where tm.team_id = rugby_data.team_id and tm.user_id = auth.uid()));
...
create policy rugby_admin_delete on rugby_data
  ...
  using (exists (select 1 from team_members tm where tm.team_id = rugby_data.team_id and tm.user_id = auth.uid() and tm.role = 'admin'));
...
create policy team_players_member_all on team_players
  ...
  using (exists (select 1 from team_members tm where tm.team_id = team_players.team_id and tm.user_id = auth.uid()))
  with check (exists (select 1 from team_members tm where tm.team_id = team_players.team_id and tm.user_id = auth.uid()));
